...
def updateTasks():
    ...
    for task in tasks:
        db_table_column = []
        db_table_column.append(task.taskname)
        db_table_column.append(task.description)
        db_table_column.append(getcategorybyid(task.category_id))
        db_table_column.append(task.priority_id)
        db_table_column.append(task.due_date)
        rowstest.append(db_table_column)
        print(rowstest)
    ...
    def __init__(self,rows):
        ...
        for col in range(1,6):
            for row in range(0,len(rowstest)):
                self.table.setItem(row,col,QTableWidgetItem(str(rowstest[0][col-1])))
        # FIX!!!!!!!!!!!!!!!!
